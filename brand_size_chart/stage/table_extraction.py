...
class TableExtractionStage:
    ...
    def _prompt_context_get(self, table_extraction: TableExtraction) -> str:
        ...
        return (
            ...
            "browser evidence write directory using absolute paths for browser tool filename arguments, and reference "
            "those artifacts in evidence_path_list with result-dir-relative paths under the evidence reference directory. "
            "Extract only the table whose browser-visible heading, tab, section, or table title matches Target "
            "size_group_key and Target source title. If the source page contains multiple chart tabs, groups, or tables, "
            "select the matching target table before reading rows. Do not extract a default, current, product-category, "
            "adjacent, or differently named table from the same page. Do not change size_group_key, source_type, "
            "source_url, or source_title. Extract the table exactly from browser-visible evidence and preserve all source "
            "columns that belong to the size chart when that column has a value in the source row. "
            "Do not emit measurement entries for blank source cells; describe systematic blank cells in the chart "
            "description when needed. For size-system or label-equivalence columns such as US/UK, IT, "
            ...
            "like size or taille. Every chart row must also preserve row.size_label exactly as one measurement with "
            "unit='size', min_value=row.size_label, and max_value=row.size_label; when the source table has a visible "
            "label-column header such as BEDENLER or SIZE, use that header as the measurement name. Set "
            "applicability_status from Source discovery country_code_list: ['GLOBAL'] means official_global, ['EU'] "
            "means official_eu_consensus, multiple explicit country codes means official_cross_locale_consensus, and "
            "one explicit country code such as ['TR'] means priority_country_official.\n\n"
