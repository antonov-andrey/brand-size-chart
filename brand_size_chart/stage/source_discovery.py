...
class SourceDiscoveryStage:
    ...
    def _prompt_context_get(self, draft_result: SourceDiscoveryResult) -> str:
        ...
        return (
            ...
            "under the browser evidence write directory. Use absolute paths under the browser evidence write directory "
            "for browser tool filename arguments; use result-dir-relative paths under the evidence reference directory "
            "inside returned JSON evidence_path_list values. "
            ...
            "URL templates. For Turkish official hosts, run separate browser-visible searches for both `beden rehberi` "
            "and `beden tablosu` before concluding that no official size-guide table exists; these terms are "
            "search-term families, not URL templates. Record the localized queries and the opened or rejected results "
            ...
            "one locale. "
            "Write each relevant browser evidence artifact under the browser evidence write directory and reference "
            "the matching result-dir-relative path under the evidence reference directory in evidence_path_list. "
            "Return every unique size-chart source candidate backed by evidence files. "
            ...
            "alone. The candidate size_group_key must match one concrete browser-visible chart group or table heading "
            "on the opened source surface; if an opened product page belongs to socks but its visible size-chart modal "
            "only exposes an Ayakkabi table, return the visible shoes table or reject the socks candidate with an "
            "explicit inventory reason instead of inventing women_socks. "
