...
Version `0.7.3` is the only current source release. `versions.yaml` contains only its project identity, SemVer and current contract major map. The pre-production platform cutover recreates Workflow and input state directly on this schema; historical input shapes, migration edges, migration scripts, compatibility fields and runtime fallbacks are absent.
