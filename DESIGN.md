...
Version `0.5.0` is the single incompatible input successor to `0.4.0`; `versions.yaml` retains the exact edge `0.4.0 -> 0.5.0` through `migration/input/0.4.0_to_0.5.0.py`. Version `0.6.0` performs the destructive platform-interface cutover to major 2 and advances `contracts.artifact_schema` to `4` for the public Data and dataset contract. Version `0.7.0` completes the browser/VPN runtime split, declares only `browser_runtime`, and adds the inherited explicit nullable `mcp_playwright_network_proxy_name` to each Codex step config so the platform can provision the union of exact named proxies without selecting one for the workflow. Version `0.7.1` keeps that public input contract and ensures that every declared brand manifest root exists before the durable brand safepoint, including a result root with no chart files. Version `0.7.2` keeps the exact `0.7.1` input, runtime, and artifact contracts under a new immutable release identity. Version `0.7.3` additionally guarantees that a brand which produces no valid chart still publishes the exact declared zero-row JSON Lines dataset before its durable safepoint, so Athena projection never depends on a missing staged dataset path. The pre-production platform cutover recreates Workflows on this schema, so no `0.6.x -> 0.7.x` input migration or compatibility field exists.
