...
Version `0.7.3` is the only current source release. `versions.yaml` contains only its project identity, SemVer and current contract major map because this source has no current migration edge. The pre-production platform cutover recreates Workflow and input state directly on this schema; historical input shapes, concrete migration edges, migration scripts, compatibility fields and runtime fallbacks are absent. The shared contract can still execute a future source-owned migration declared by a later current release without restoring one removed `brand-size-chart` implementation.
...
Each accepted canonical chart is copied to `/result/{brand_key}/size_chart/`, and its flat Athena-visible measurement rows are written atomically to `/result/{brand_key}/dataset/brand_size_chart/part-00000.jsonl`. Every row contains the exact immutable `workflow_run_id`, `workflow_run_timestamp`, and `workflow_source_version_id` provenance from `WorkflowRunContext`. Before requesting the brand safepoint, the workflow creates both declared `/result/{brand_key}` and `/workspace/{brand_key}` manifest roots even when domain processing produced no chart files. The brand safepoint writes `/workspace/{brand_key}/safepoint.json` and requests both declared manifests with stable `step_key: brand_complete`; the final request carries only the open `RunResult` because each brand manifest was already accepted.
