...
Codex processes one requested product type completely before moving to the next. An explicitly sexed product type has one search sex. An explicitly unisex or unsexed adult product type has the ordered search-sex list `[men, women]`; an explicitly unisex or unsexed child or baby product type has `[boys, girls]`. Every list member is processed; the order is not fallback behavior. Each worklist entity has a row-local FSM: Codex appends its first `pending` revision immediately before searching, then appends the next revision as terminal `searched` after inspecting an applicable concrete product boundary or terminal `rejected` when no authorized matching boundary can be searched. Codex never creates all initial rows in advance and does not start another row before the current entity is terminal.

For each worklist row, Codex records one bounded query, opens a concrete matching product when the pair is applicable, inspects every product-local size surface, and saves each completed query, URL, and table decision before continuing. A terminal `searched` row has current evidence and an evidence-backed outcome reason. A terminal `rejected` row has current evidence and an evidence-backed reason. A `searched` row also has one dedicated opened URL record whose `worklist_key_list` contains only that row key; that linkage validates evidence and source-boundary closure, not worklist state. Table inventory rows do not close worklist rows.
...
No accepted table is a valid non-fatal result only when the private inventory contains evidence-backed terminal reasons, including terminal `searched` and `rejected` worklist outcomes. Python derives `SourceDiscoveryResult.warning_list` from that inventory during the same step lifecycle. An empty accepted table set does not require `table.jsonl` when every required worklist row is terminal and its reasons are complete. Downstream workflows consume only the public result and never read the private JSONL files.
