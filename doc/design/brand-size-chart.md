...
For Turkish official hosts, `source_discovery` must run separate browser-visible searches for both `beden rehberi` and `beden tablosu` before concluding that no official size-guide table exists. These are localized search-term families, not URL templates.
...
`table_extraction` must fail after semantic verification when `source_type`, `source_url`, `source_title`, or `size_group_key` differs from the verified `SourceDiscovery`, one evidence reference is missing or outside the result directory, the chart has no rows, one row has empty `size_label`, one row has no measurements, or one measurement has empty `name`, `unit`, `min_value`, or `max_value`.
...
Browser-stage prompts must expose two evidence paths: an absolute browser evidence write directory for Playwright MCP tool `filename` arguments, and a result-dir-relative evidence reference directory for returned `evidence_path_list` values. Browser tools must not receive result-dir-relative filenames, because Playwright MCP resolves such filenames outside the workflow output root.
